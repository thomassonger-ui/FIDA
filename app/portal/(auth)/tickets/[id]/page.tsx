...
export default async function PortalTicketDetail({
  ...
  if (normalizeEmail(ticket.email) !== normalizeEmail(user.email)) return notFound();
  ...
  return (
    <div className="max-w-4xl">
      <div className="text-xs text-subtle mb-4">
        <Link href="/portal/tickets" className="hover:text-teal">← Your tickets</Link>
      </div>
      ...
      {canReply ? (
        ...
        <div className="mt-10 card p-5 text-sm text-muted">
          This ticket is closed. <Link href="/portal/tickets/new" className="text-teal hover:underline">Open a new one</Link> if you need more help.
        </div>
      )}
    </div>
  );
